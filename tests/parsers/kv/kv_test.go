...
func TestParser(t *testing.T) {
	...
	res, success = kv.Parse(semicolonEqualInput, semicolon, equal)
	assert.DeepEqual(t, success, true)
	assert.DeepEqual(t, res["key1"], []byte("value1"))
	assert.DeepEqual(t, res["key2"], []byte("value2"))

	res, success = kv.Parse(colonEqualInput, colon, equal)
	assert.Assert(t, success, true)
	assert.DeepEqual(t, res["key1"], []byte("value1"))
	assert.DeepEqual(t, res["key2"], []byte("value2;"))

	res, success = kv.Parse(minusCommaInput, minus, comma)
	assert.Assert(t, success, true)
	assert.DeepEqual(t, res["key1"], []byte("value1"))
	assert.DeepEqual(t, res["key2"], []byte("value2"))

	res, success = kv.Parse(spacedInput, comma, equal)
	assert.Assert(t, success, true)
	assert.DeepEqual(t, res["first key"], []byte("value1"))
	assert.DeepEqual(t, res["second key"], []byte("value2"))

	res, success = kv.Parse(escapedInput, comma, equal)
	assert.Assert(t, success, true)
...
func BenchmarkParser(b *testing.B) {
	b.ReportAllocs()
	for i := 0; i < b.N; i++ {
		kv.Parse(benchmarkInput, semicolon, equal)
	}
}
