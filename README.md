...
First of all, [collector](#collector) needs to actively fetch or passively receive raw logs. For that purpose various [connectors](#connector) are used. After log record was grabbed by a connector, it is passed to one of the processing workers in a round-robin fashion. Inside processing worker a log record is passed to a [normalizer](#normalizer), which parses and converts it to a [normalized event](#normalized-event) according to the user-defined [mapping rules](#mapping-rule). Next, a [normalized event](#normalized-event) is passed through the [filters](#filter), if any defined. If an event did not pass all the [filters](#filter), it is dropped. Otherwise an event is passed through the [enrichment rules](#enrichment-rule) and [aggregation rules](#aggregation-rule), if any defined. If an event wasn't aggregated, it is sent to each [destination](#destination) defined by user immediately. Otherwise it will be sent after the aggregation timeout exceeded or aggregation threshold reached. 
